...
RegisterNetEvent("trucker:listTheJobs")
AddEventHandler("trucker:listTheJobs")

RegisterCommand('truckerjoblist', function(source, args, user)
  TriggerEvent('trucker:returnCurrentJobs')
end)

RegisterNetEvent('server:givepayJob')
AddEventHandler('server:givepayJob', function(info,amount)
  local src = source
