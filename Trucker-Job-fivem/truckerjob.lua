  ...
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      ...
      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["framework-taskbar"]:taskBar(25000,"Storing Trailer Contents")
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["framework-base"]:getModule("LocalPlayer"):getVar("job") == "trucker" and existingVeh ~= nil then
      ...
      elseif job ~= 0 and stage == "drop" then
        ...
        elseif currentJobs[currentJobPos].JobType ~= "Shops" then
          ...
          if dist < 20 then
            ...
            payment = math.random(150,payment)
            TriggerServerEvent("server:givepayJob:truck", "Delivery", payment)
          end
          Wait(1)
        end
      elseif job ~= 0 and stage == "waiting" then
        if waitingForConfirmation then
          ...
          if math.random(150) > 66 then
            --
            TriggerEvent("payment:chopshopscrap",math.random(30),false)
          end

          TriggerServerEvent("server:givepayJob:truck", "Delivery Service", math.random(100,payment))
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["framework-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            ...
            TriggerServerEvent("framework-shops:checkOwner",GetPlayerServerId(t),currentJobs[currentJobPos].shopId)
        else
...
RegisterNetEvent("framework-base:characterLoaded")
AddEventHandler("framework-base:characterLoaded", function()
...
function spawnTruck(i)
      ...
      local finished = exports["framework-taskbar"]:taskBar(3000,"Checking Vehicle")
      if finished == 100 then
      SetVehicleEngineHealth(spawned_car, 1000.0)
      SetVehicleBodyHealth(spawned_car, 1000.0)
      SetVehiclePetrolTankHealth(spawned_car, 1000.0)
